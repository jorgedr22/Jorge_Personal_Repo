...
void InitI2C(){
  ...
  // TWBR = ((CPU clock frequency)/(SCL frequnecy) - 16)/(2*(4^TWPS))
...
// this function initiates a start condition and calls slave device with SLA
  TWCR = (1<<TWINT)|(1<<TWSTA)|(1<<TWEN); // clear TWINT, initiate a start condition and enable
  wait_for_completion;
...
void write(unsigned char data){
  ...
  TWCR = (1<<TWINT)|(1<<TWEN);  // trigger action:  clear flag and enable TWINT
  wait_for_completion;
}
