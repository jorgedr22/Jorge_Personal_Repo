...
inline static uint8_t spiTransfer(unsigned char data) {
...
void MFRC522::PCD_WriteRegister(	PCD_Register reg,	///< The register to write to. One of the PCD_Register enums.
	...
	spiTransfer(reg);						// MSB == 0 is for writing. LSB is not used in address. Datasheet section 8.1.2.3.
	spiTransfer(value);
...
byte MFRC522::PCD_ReadRegister(	PCD_Register reg	///< The register to read from. One of the PCD_Register enums.
	...
	// set SS low to begin SPI frame
    PORTB &= ~(1<< PB0); // Select MFRC522
	spiTransfer(0x80 | reg);					// MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.
...
byte* MFRC522::PICC_GetUid(Uid *uid){
    return uid->uidByte;
}

byte MFRC522::PICC_GetSize(Uid *uid){
    return uid->size;
}
